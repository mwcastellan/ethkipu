...
contract ToDoList {
    ...
    function setTarea(string calldata _description) external {
        ...
        tarea.push(
            Tarea(_description, block.timestamp, _lastIndex, State.SinHacer)
        );
    ...
    function eliminarTarea(string calldata _descripcion) external {
        ...
        for (uint256 i; i < len; ) {
            if (
                keccak256(bytes(tarea[i].description)) ==
                keccak256(bytes(_descripcion))
            ) {
                emit TaskedStatusChanged(
                    tarea[i].index,
                    tarea[i].description,
                    "eliminado"
                );
